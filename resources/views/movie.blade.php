...
@section('content')
    ...
    <div class="w-full h-screen relative">
        <div
            ...
            <div
                class="absolute top-10 right-10 bg-black bg-opacity-70 text-white p-10 flex flex-col rounded-3xl md:grid md:grid-cols-3070 gap-5 auto-rows-auto md:w-[75%] h-fit sm:w-[85%]">
                <img src="https://image.tmdb.org/t/p/w500{{$movie['poster_path']}}"
                     class="rounded shadow shadow-gray-700 h-56 w-40 col-start-2 row-span-full" alt="poster">
                <div class="flex flex-col justify-end items-end">
                    <h1 class="text-2xl font-bold  col-start-1 text-right">{{$movie['title']}}</h1>
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-yellow-600 text-right" fill="none"
                         viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round"
                                                                         stroke-width="2"
                                                                         d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>{{$movie['vote_average']*10}}%
                    </span>
                    <div class="flex flex-row-reverse"><label class="font-bold text-lg w-32 ">: تاریخ انتشار</label>
                        <span>{{$movie['release_date']}}</span></div>
                    <p class="text-sm md:block hidden">{{$movie['overview']}}</p>
    ...
    <p class="md:hidden block text-white text-right mx-5 mb-10">{{$movie['overview']}}</p>
    <hr>
    <h1 class="text-2xl text-white m-3 uppercase">Actor</h1>
        <div class="grid grid-cols-1 my-10 justify-items-center sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 lg:my-10">
            @foreach($movie['credits']['cast'] as $actor)
                @if($loop->index <6)
                    <div class="flex flex-col justify-center items-center sm:my-3 lg:my-1">
                        <x-card>
